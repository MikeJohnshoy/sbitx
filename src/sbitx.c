// ************************************************************
// sbitx.c
// An improved version of the sBitx application and operating 
// system for the HF Signals sBitx hardware.
//
// GNU General Public License version 3  [IS THIS RIGHT?]
// ************************************************************
...
#define PLANTIME - 1 // spend no more than plantime seconds finding the best FFT algorithm. -1 turns the plantime cap off.
...
void setup_audio_codec() {    //configure all the channels of the mixer
...
void setup_oscillators() {    //initialize the SI5351
  delay(200);
  si5351bx_init();
  delay(200);
...
static int hw_settings_handler(void * user,
           const char * section,
           const char * name,
           const char * value) {
  char cmd[1000];
...
void rx_linear(int32_t * input_rx, int32_t * input_mic,
  ...
  if (r -> mode != MODE_DIGITAL && r -> mode != MODE_FT8 && r -> mode != MODE_2TONE) {
    ...
    static double noise_est[MAX_BINS] = {
      0
    };
    static double signal_est[MAX_BINS] = {
      0
    }; // For Wiener filter
...
void wav_record(int32_t * samples, int count) {
  ...
  w = record_buffer;
  while (i < count) {
    * w++ = * samples / 32786; // SHOULD THIS BE 32768 (2^15)?
...
//Write code that must repeatedly do things, it is called during the idle time 
//of the event loop 
void loop() {
  delay(10);
}
...
void setup() { // This is the one-time initialization code
